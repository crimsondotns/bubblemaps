...
for count in [150, 250, 300]:
    ...
    for rounds in [0, 1, 2]:
        if rounds == 0:
            ...
            # 2 rounds of magic expand
            url2 = f"{API_BASE}/addresses/expand/magic"
            h2 = {**HEADERS, "x-validation": _make_validation("/addresses/expand/magic")}
            resp2 = requests.post(url2, json={"addresses": addrs, "token_ref": {"chain": chain, "address": token_addr}}, headers=h2)
            magic1 = resp2.json() if isinstance(resp2.json(), list) else resp2.json().get("addresses", [])
            
            # Expand magic1 nodes too
            magic1_addrs = [x["address"] for x in magic1]
            resp3 = requests.post(url2, json={"addresses": magic1_addrs, "token_ref": {"chain": chain, "address": token_addr}}, headers=h2)
            magic2 = resp3.json() if isinstance(resp3.json(), list) else resp3.json().get("addresses", [])
            all_holders = holders + magic1 + magic2
        ...
        for showCex in [False, True]:
            for showDex in [False, True]:
                for showContracts in [False, True]:
                    ...
                    # Compute visible addresses using exact z6 logic
                    visible = set()
                    ...
                    # Calculate share only for nodes that are inside the visible cluster and hold the token (share > 0)
                    ...
                    if 70 <= len(cluster_members) <= 90 or 0.04 <= total_share <= 0.05:
                        print(f"MATCH: cnt={count} rnds={rounds} C={showCex} D={showDex} K={showContracts} | Cluster Size: {len(cluster_members)} | Supply: {total_share*100:.2f}%")
